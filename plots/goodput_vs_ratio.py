...
def plot_goodput_vs_ratio(topo="random"):
    ...
    for ratio, (request_num, goodput) in results.items():
        ax.plot(request_num, goodput, linewidth=1.0, label=str(ratio))
    ax.set_xlabel('Number of Requests (S-D Pairs)')
    ax.set_ylabel('Goodput (ebits/s)')
    ax.grid(True)
    ax.legend(title="Benchmarking Ratio", fontsize=14, title_fontsize=18)
    plt.tight_layout()
    plt.savefig(f"plot_goodput_vs_ratio_{topo}_topo.pdf")
    # plt.show()
    ...
    while index <= request_num:
        ...
        for i in range(repeat):
            ...
            goodput += network.simulate_traffic("Poisson",
                                                ...
                                                index,
                                                with_benchmark=True,
                                                random_pairs=as_ip_pairs_all[0:index],
