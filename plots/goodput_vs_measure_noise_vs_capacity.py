...
def plot_goodput_vs_measure_noise_vs_capacity():
    """
    X axis: the Bell measurement success rate (i.e., the swapping success rate)
    ...
    file_path = os.path.join(output_dir, "plot_goodput_vs_measure_noise_vs_capacity.pickle")
    ...
    plt.rc("axes", prop_cycle=default_cycler)
    fig, ax = plt.subplots()
    ax.set_xlabel("Average Swapping Noise Rate")
    ...
    filename = os.path.join(figure_dir, "plot_goodput_vs_measure_noise_vs_capacity.pdf")
...
def evaluate(capacity):
    ...
    repeat = 20
    while measure_noise_rate < 1.0:
        seed = 872
        set_random_seed(seed)
        network = QuantumNetwork(gate_noise_rate=measure_noise_rate)
        ...
        goodput /= repeat
        x.append(measure_noise_rate)
        y.append(goodput)
        measure_noise_rate += 0.05
