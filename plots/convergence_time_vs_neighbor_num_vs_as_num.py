...
def plot_convergence_time_vs_neighbor_num_vs_as_num():
    """
    X axis: the number of ASes
    Y axis: convergence time (the time it takes to converge to a stable state)
    Bar: the maximum number of neighbors for each AS
    """
    ...
    file_path = os.path.join(output_dir, "plot_convergence_time_vs_neighbor_num_vs_as_num.pickle")
    ...
    if os.path.exists(file_path):
        ...
        neighbor_num_list = [2, 3, 4, 5, 6]
        results = []
        for neighbor_num in neighbor_num_list:
            results.append(p.apply_async(evaluate, args=(neighbor_num,)))
        p.close()
        p.join()
        results = {neighbor_num_list[i]: r.get() for i, r in enumerate(results)}
        print(results)
        # results_dict = {}
    ...
    ax.legend(title="Maximum Number of Neighbors", fontsize=20, title_fontsize=20, ncols=3)
    plt.tight_layout()
    if not os.path.exists(figure_dir):
        os.makedirs(figure_dir)
    filename = os.path.join(figure_dir, "plot_convergence_time_vs_neighbor_num_vs_as_num.pdf")
    plt.savefig(filename)
    os.system("pdfcrop" + " " + filename + " " + filename)


def evaluate(max_neighbors_num):
    seed = 88
    # seed = 811
